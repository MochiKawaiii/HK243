...
class TicTacToeGame:
    def __init__(self, root):
        ...
        """
        self.root = root
        self.root.title("Game XO 4x4 (Refactored)")
    ...
    def check_winner(self, row, col):
        ...
        # 3. Kiểm tra đường chéo chính (chỉ khi ô nằm trên đường chéo)
        if row == col:
            if all(self.board[i][i] == player for i in range(self.BOARD_SIZE)):
                return True
                
        # 4. Kiểm tra đường chéo phụ (chỉ khi ô nằm trên đường chéo)
